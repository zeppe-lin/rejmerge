...
temporary file will later be presented to the user as the merge
result.
...
purpose is to provide information about the merge, e.g.  "5 merge
conflicts found".
...
C<PAGER> was not set from command-line.  C<-R> option is required if
diffs above produces raw control chars:

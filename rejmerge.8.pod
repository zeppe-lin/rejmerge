...
B<rejmerge> (unless F</etc/rejmerge.conf> override them):
...
The original implementation of B<rejmerge> was done by Per Lidén
...
Please note that running C<rejmerge -n> as a user, you may encounter
that the rejected file has stricter permissions (e.g., 0400) that
prohibit the users from reading its content:

 $ PAGER=/bin/cat rejmerge -n
 /usr/bin/diff: /var/lib/pkg/rejected/etc/tor/torrc: Permission denied
 =======> /etc/tor/torrc
 =======> [K]eep [U]pgrade [M]erge [D]iff [S]kip?
 $

Run C<rejmerge> as root if you want to view it.

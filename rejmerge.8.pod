...
Specify the pathname of an alternative root directory; the default is
C</>.  This should be used if you want to merge rejected files on a
...
Default: I<PATH>F</etc/rejmerge.conf> if B<-r>/B<--root> is specified
and F</etc/rejmerge.conf> otherwise.

=item B<-n>, B<--dry-run>

Print the differences between an installed version and rejected
version, but do not execute any actions.
